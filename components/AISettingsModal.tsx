...
import { X, Plus, Trash2, CheckCircle2, Circle, Loader2, Power, AlertTriangle } from 'lucide-react';
              ...
              {aiModels.map((model) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`text-purple-600 ${activeModel === model ? 'opacity-100' : 'opacity-30'}`}>
                        {activeModel === model ? <CheckCircle2 size={20} /> : <Circle size={20} />}
                    </div>
                    <span className={`text-sm font-medium ${activeModel === model ? 'text-purple-900' : 'text-gray-700'}`}>
                      {model}
                    </span>
                  </div>
                  
                  {/* We don't need delete for detected models usually, but keeping it if user added manually or wants to hide it from list. 
          ...
          <div className="pt-4 border-t border-gray-100">
            ...
            <form onSubmit={handleAdd} className="flex gap-2">
              <input
                ...
                className="flex-1 text-sm px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
              <button 
                type="submit"
        ...
        {!isAIEnabled && (
            <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end">
                <button
                    ...
                    className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg font-medium transition shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                >
