                        ...
                        <tr>
                            <th scope="col" className="px-6 py-4 w-10 border-b border-gray-200 bg-gray-50">
                                <div
                                    ...
                                    className={`w-5 h-5 rounded border transition-all flex items-center justify-center cursor-pointer ${allSelected
                            ...
                            tasks.map((task) => {
                                ...
                                return (
                                    <tr
                                        key={task.id}
                                        onClick={() => onEdit(task)}
                                        className={`kanban-table-row border-b transition-colors cursor-pointer group ${isSelected ? 'bg-blue-50/50' : 'bg-white hover:bg-gray-50'
                                            }`}
                                    >
                                        <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                                            <div
                                                onClick={() => !isReadOnly && onToggleTaskSelection && onToggleTaskSelection(task.id)}
                                                className={`w-5 h-5 rounded border transition-all flex items-center justify-center ${isReadOnly ? 'cursor-default opacity-50 bg-gray-50' : 'cursor-pointer'
                                                    } ${isSelected
                                                        ? 'bg-blue-600 border-blue-600 text-white shadow-sm'
                                                        : 'bg-white border-gray-300 text-transparent hover:border-blue-400'
                                                    }`}
                                        ...
                                        <td className="px-6 py-4 text-center" onClick={(e) => e.stopPropagation()}>
                                            <button
                                                ...
                                                className={`btn-toggle-check transition ${isReadOnly ? 'cursor-default' : 'text-gray-400 hover:text-green-600'}`}
                                            >
                                                {task.isCompleted ? <CheckCircle2 className="text-green-600" size={20} /> : <Circle className={isReadOnly ? 'text-gray-300' : 'text-gray-400'} size={20} />}
                                            </button>
                                        </td>
