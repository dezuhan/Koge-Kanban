...
export const TaskCard: React.FC<TaskCardProps> = ({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`kanban-task-card relative bg-white p-3 rounded-lg shadow-sm border group hover:shadow-md transition-all cursor-grab active:cursor-grabbing ${isSelected ? 'border-blue-500 bg-blue-50/30 ring-1 ring-blue-500/20' : 'border-gray-200'
    ...
    >
      ...
      <div
        className={`absolute top-3 left-3 z-10 w-[22px] h-[22px] flex items-center justify-center rounded border transition-all pointer-events-auto ${isSelected
          ...
          } ${isReadOnly ? 'cursor-default opacity-60 grayscale' : ''}`}
        onPointerDown={(e) => e.stopPropagation()}
        onClick={(e) => {
          e.stopPropagation();
          if (!isReadOnly && onToggleTaskSelection) onToggleTaskSelection(task.id);
        }}
      >
      ...
      <div className={`kanban-task-header flex justify-between items-center mb-2 ${task.media && task.media.length > 0 ? 'mt-3' : 'mt-0'}`}>
        ...
        <div className="kanban-task-actions flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            ...
            title={isReadOnly ? "View Details" : "Edit Task"}
          >
            <Edit2 size={14} />
          </button>
          {!isReadOnly && (
      ...
      <div className="kanban-task-footer flex items-center justify-between mt-2 pt-2 border-t border-gray-50 text-xs text-gray-400">
        ...
        <button
          onPointerDown={(e) => e.stopPropagation()}
          onClick={(e) => { e.stopPropagation(); !isReadOnly && onToggleCheck(task.id); }}
          className={`btn-toggle-check transition-colors ${task.isCompleted ? 'text-green-500' : 'text-gray-300 hover:text-gray-500'} ${isReadOnly ? 'cursor-default opacity-60' : ''}`}
          disabled={isReadOnly}
        >
