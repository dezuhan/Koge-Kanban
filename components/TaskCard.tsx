...
import { Edit2, Trash2, CheckSquare, Square, Calendar, CheckCircle2, Image as ImageIcon, Copy } from 'lucide-react';
...
export const TaskCard: React.FC<TaskCardProps> = ({ 
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`kanban-task-card bg-white p-3 rounded-lg shadow-sm border border-gray-200 group hover:shadow-md transition-all cursor-grab active:cursor-grabbing ${isDragging ? 'opacity-50 ring-2 ring-blue-500 rotate-2 z-50' : ''}`}
      {...attributes}
      {...listeners}
    ...
    >
      ...
      <div className={`kanban-task-header flex justify-between items-start mb-2 ${task.media && task.media.length > 0 ? 'mt-3' : ''}`}>
        <span 
